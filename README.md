...
项目借鉴了 [CookHero](https://github.com/Decade-qiu/CookHero) 的架构设计，在此基础上进行了全面重构与功能扩展，目前仍在持续完善中。
...
│                                                          │
│  ┌──────────┐  ┌────────────────┐  ┌─────────────────┐  │
│  │  Auth    │  │  Conversation  │  │  ReAct Agent    │  │
│  │  Service │  │  Service (RAG) │  │  Service        │  │
│  └──────────┘  └───────┬────────┘  └───────┬─────────┘  │
│                        │                    │             │
│              ┌─────────┴────────┐  ┌───────┴─────────┐  │
│              │   RAG Pipeline   │  │  Tool Functions │  │
│              │  · Hybrid Search │  │  · 6 Built-in   │  │
│              │  · Small2Large   │  │  · Extensible   │  │
│              │  · Dual Cache    │  │                 │  │
│              └────────┬─────────┘  └─────────────────┘  │
│                       │                                   │
│  ┌────────────────────┼──────────────────────────────┐  │
...
| **Agent 模式** | ReAct 循环（Thought → Action → Observation） | 工具调用 + 多步推理，最多 8 轮迭代 |
...
│   ├── auth/                       # 认证模块（注册/登录/JWT）
│   ├── agent/                      # ReAct Agent（核心循环 + 6 工具）
│   │   └── tools/                  # 工具函数集合
│   ├── conversation/               # RAG 对话模块 + 意图检测
│   ├── rag/                        # RAG 管线
...
| 配置文件 | 用途 |
|---------|------|
| `config.yml` | 非敏感配置：模型名、检索参数、限流规则、压缩策略 |
...
| **Normal** | qwen-max | 最终回答生成、Agent 推理 |
